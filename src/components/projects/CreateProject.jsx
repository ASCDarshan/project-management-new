...
const CreateProject = () => {
  ...
  const { createProject, categories, employees, loading } = useProject();
  ...
  useEffect(() => {
    // Calculate selected tasks preview
    const tasks = [];
    Object.entries(formData.selectedTasks).forEach(([categoryId, subcategories]) => {
  ...
  const handleCategoryToggle = (categoryId) => {
    ...
    setFormData(prev => {
      ...
      if (isSelected) {
        ...
        const newSelectedCategories = prev.selectedCategories.filter(id => id !== categoryId);
        ...
        category.subcategories?.forEach(subcategory => {
          newSelectedTasks[categoryId][subcategory.name] = subcategory.tasks.map(task => ({
            name: task,
  ...
  const handleTaskToggle = (categoryId, subcategoryName, taskIndex) => {
    setFormData(prev => {
      const newSelectedTasks = { ...prev.selectedTasks };
      if (newSelectedTasks[categoryId] && newSelectedTasks[categoryId][subcategoryName]) {
        ...
        return (
          <Box>
            ...
            <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
              Select the categories that apply to your project. Default tasks will be included for each category.
            </Typography>
                ...
                <Grid item xs={12} key={category.id}>
                  ...
                  >
                    <CardContent>
                      ...
                      <Collapse in={formData.selectedCategories.includes(category.id)}>
                        <Box sx={{ ml: 4 }}>
                            ...
                            <Box key={subcategory.name} sx={{ mb: 2 }}>
                              ...
                              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                                {subcategory.tasks.map((task, taskIndex) => {
                                  const isSelected = formData.selectedTasks[category.id]?.[subcategory.name]?.[taskIndex]?.selected;
                                  return (
                                    <Chip
                                      key={taskIndex}
                                      label={task}
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        handleTaskToggle(category.id, subcategory.name, taskIndex);
                                      }}
                                      sx={{
      ...
      case 2:
        return (
          <Box>
            ...
            {employees.length === 0 && (
              <Alert severity="info">
                No team members found. You can add team members later from the employees section.
              </Alert>
            )}
          </Box>
        );
          ...
          <Box>
            ...
            <Grid container spacing={3}>
              <Grid item xs={12} md={6}>
                <Paper elevation={0} sx={{ p: 2, backgroundColor: 'rgba(139, 126, 200, 0.05)' }}>
                  <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                    Project Details
                  </Typography>
                  <Typography variant="body2"><strong>Name:</strong> {formData.name}</Typography>
                  <Typography variant="body2"><strong>Status:</strong> {statusOptions.find(s => s.value === formData.status)?.label}</Typography>
                  <Typography variant="body2"><strong>Priority:</strong> {priorityOptions.find(p => p.value === formData.priority)?.label}</Typography>
                  {formData.dueDate && (
              ...
              <Grid item xs={12} md={6}>
                <Paper elevation={0} sx={{ p: 2, backgroundColor: 'rgba(139, 126, 200, 0.05)' }}>
                  ...
                  <Typography variant="body2">
                    <strong>Categories:</strong> {formData.selectedCategories.length}
                  </Typography>
                  <Typography variant="body2">
      ...
      <Box>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={4}>
            <Paper elevation={0} sx={{ p: 3, height: 'fit-content', position: 'sticky', top: 20 }}>
                      ...
                      StepIconComponent={() => (
                        <Box
                          sx={{
                            ...
                            backgroundColor: index <= activeStep ? '#8B7EC8' : 'rgba(139, 126, 200, 0.2)',
