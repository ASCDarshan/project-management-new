// src/components/tasks/CategoryManager.jsx
...
const CategoryManager = () => {
  const { categories, loadCategories } = useProject();
  
  const [selectedCategory, setSelectedCategory] = useState(null);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [dialogType, setDialogType] = useState(''); // 'category', 'subcategory', 'task'
  ...
  const colorOptions = [
    '#8B7EC8', '#B5A9D6', '#A8E6CF', '#FFD3A5', '#FFAAA5', 
    '#A5C9FF', '#E6E6FA', '#F0F0FF', '#D1C4E9', '#C8F2D8'
  ];
  ...
  const handleSaveSubcategory = () => {
    // Here you would typically save to Firebase
    console.log('Saving subcategory:', formData, 'to category:', selectedCategory);
    handleCloseDialog();
  };
  ...
  const getTotalTasks = (category) => {
    ...
    return category.subcategories.reduce((total, sub) => total + (sub.tasks?.length || 0), 0);
  };
      ...
      <Box>
        ...
        >
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={6} sm={3}>
              <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, backgroundColor: 'white' }}>
                <Typography variant="h3" fontWeight={700} sx={{ color: '#A8E6CF' }}>
            ...
            <Grid item xs={6} sm={3}>
              <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, backgroundColor: 'white' }}>
                <Typography variant="h3" fontWeight={700} sx={{ color: '#FFD3A5' }}>
                  {categories.reduce((total, cat) => total + getTotalTasks(cat), 0)}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Task Templates
                </Typography>
            ...
            <Grid item xs={6} sm={3}>
              <Box sx={{ textAlign: 'center', p: 2, borderRadius: 2, backgroundColor: 'white' }}>
                <Typography variant="h3" fontWeight={700} sx={{ color: '#A5C9FF' }}>
                  {Math.round(categories.reduce((total, cat) => total + getTotalTasks(cat), 0) / Math.max(categories.length, 1))}
                </Typography>
                <Typography variant="body2" color="text.secondary">
                  Avg Tasks/Category
                </Typography>
        ...
        {categories.length === 0 ? (
          ...
          >
            <Category sx={{ fontSize: 60, color: 'text.secondary', mb: 2, opacity: 0.7 }} />
            <Typography variant="h5" gutterBottom sx={{ fontWeight: 600 }}>
              No categories yet
            </Typography>
            <Typography variant="body1" color="text.secondary" sx={{ mb: 3, maxWidth: 400, mx: 'auto' }}>
              Create your first category to start organizing tasks with custom templates and workflows.
            </Typography>
            <Button
              variant="contained"
              startIcon={<Add />}
                      ...
                      <AccordionDetails sx={{ p: 0, pt: 0 }}>
                        ...
                        <Box sx={{ p: 3 }}>
                          {category.subcategories?.map((subcategory) => (
                            ...
                            >
                              <CardContent sx={{ p: 2 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1 }}>
                                    <Button
                                      size="small"
                                      startIcon={<Add />}
                                      onClick={() => handleOpenDialog('task', category, subcategory)}
                                    ...
                                    <Button
                                      size="small"
                                      startIcon={<Edit />}
                                      onClick={() => handleOpenDialog('subcategory', category, subcategory)}
                                      sx={{ textTransform: 'none' }}
                                    >
                                      Edit
                                    </Button>
                                ...
                                {subcategory.tasks && subcategory.tasks.length > 0 ? (
                          ...
                          <Button
                            ...
                            variant="outlined"
                            startIcon={<Add />}
                            onClick={() => handleOpenDialog('subcategory', category)}
                            sx={{
                              mt: 2,
                              borderColor: category.color,
                              color: category.color,
        ...
        >
          <MenuItem onClick={() => { handleOpenDialog('category', selectedCategory); handleMenuClose(); }}>
            <Edit fontSize="small" sx={{ mr: 1 }} />
            Edit Category
          </MenuItem>
          <MenuItem onClick={() => { handleOpenDialog('subcategory', selectedCategory); handleMenuClose(); }}>
        ...
        <Dialog open={dialogOpen} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
          <DialogTitle>
            {dialogType === 'category' && (selectedCategory ? 'Edit Category' : 'New Category')}
          ...
          <DialogActions>
            ...
            <Button
              variant="contained"
              onClick={
                dialogType === 'category' ? handleSaveCategory :
                dialogType === 'subcategory' ? handleSaveSubcategory :
                handleSaveTask
              }
              disabled={
                (dialogType === 'category' || dialogType === 'subcategory') ? !formData.name.trim() :
                !taskForm.name.trim()
              }
            >
              {selectedCategory && dialogType === 'category' ? 'Update' : 'Create'}
            </Button>
          </DialogActions>
        </Dialog>
      </Box>
